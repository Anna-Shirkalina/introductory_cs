...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        for i, child in enumerate(self.children):
            child._update_children_positions(children_positions[i])
        return None
    ...
    def smash(self) -> bool:
        ...
        child_max_depth = self.max_depth
        child_size = self._child_size()
        self.colour = None
        for i in range(4):
            j = random.randint(0, len(COLOUR_LIST) - 1)
            colour = COLOUR_LIST[j]
            child_block = Block(child_position[i], child_size, colour,
                                child_level, child_max_depth)
            self.children.append(child_block)
        for child in self.children:
            num = random.random()
            if num < math.exp(-0.25 * child_level):
                child.smash()
        return True
    ...
    def swap(self, direction: int) -> bool:
        ...
        if not self.children:
            return False
        position_0 = self.children[0].position
        position_1 = self.children[1].position
        position_2 = self.children[2].position
        position_3 = self.children[3].position
        if direction == 1:
            self.children[0]._update_children_positions(position_3)
            self.children[3]._update_children_positions(position_0)
            self.children[1]._update_children_positions(position_2)
            self.children[2]._update_children_positions(position_1)
            children = self.children
            self.children = [children[3], children[2], children[1], children[0]]
        else:
            self.children[0]._update_children_positions(position_1)
            self.children[1]._update_children_positions(position_0)
            self.children[3]._update_children_positions(position_2)
            self.children[2]._update_children_positions(position_3)
            children = self.children
    ...
    def combine(self) -> bool:
        ...
        if self.level != self.max_depth - 1:
            ...
            colours = self._children_colours()
            name_colours = list(colours.keys())
            num_colours = list(colours.values())
            if len(num_colours) == 4:
                return False
            if len(num_colours) == 2:
                if colours[name_colours[0]] == colours[name_colours[1]]:
                    return False
                if colours[name_colours[0]] > colours[name_colours[1]]:
                    self.colour = name_colours[0]
                    self.children = []
                else:
                    self.colour = name_colours[1]
                    self.children = []
            else:
                i = num_colours.index(max(num_colours))
                colour = name_colours[i]
                self.colour = colour
                self.children = []
        return True

    def _children_colours(self) -> Dict[Tuple[int, int, int]: int]:
        """Return a dictionary where each key is the colour of one of the
        block's children and each value is the number of children
        of that colour
        pre-condition: self.children == []
        """
        colours = {}
        for child in self.children:
